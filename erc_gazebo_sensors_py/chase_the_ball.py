...
class ImageSubscriber(Node):
        ...
        while rclpy.ok():
            ...
            if self.latest_frame is not None:
                with self.frame_lock:
                    ...
                    self.latest_frame = None  # Clear the frame after grabbing it
                ...
                # Cap the display size so the window never appears huge,
    ...
    def add_small_pictures(self, img, small_images, size=None):
        ...
        if size is None:
            # Auto-size thumbnails so all n of them fit across the frame width
            avail_w = img_w - x_base_offset * (n + 1)
            thumb_w = max(20, avail_w // n)
            thumb_h = int(thumb_w * 0.75)  # keep a 4:3-ish aspect ratio
        ...
        for small in small_images:
            ...
            # Clip to whatever room is left, just in case
            avail_w = max(0, img_w - x_offset)
            avail_h = max(0, img_h - y_offset)
            w = min(size[0], avail_w)
            h = min(size[1], avail_h)

            if w <= 0 or h <= 0:
                break

            img[y_offset: y_offset + h, x_offset: x_offset + w] = small[:h, :w]
